...
(function ($) {
  if ($ === null || $ === undefined) {
    throw new ReferenceError(
      "This plugin require javascript to run correctly."
    );
  }
  ...
  $.fn.pdfViewer = function (url, options) {
    ...
    let rawPdfData = null;
    if (settings.isBase64) rawPdfData = convertDataURIToBinary(url);
    ...
    $(this).append(
      '<div class="pdf-toolbar">' +
        '<span class="pdf-toolbar-title" id="doc-title">Menú</span>' +
    ...
    pdfjsLib
      ...
      .promise.then((doc) => {
        ...
        //   if (metadata.info.Title) $("#doc-title").text(metadata.info.Title);
        // });
    ...
    const renderPage = (page) => {
      ...
      if (
        ...
        initialState.pdfDoc
          ...
          .getPage(initialState.currentPage)
          .then(renderPage);
      } else initialState.currentPage = 1;
    };
    ...
    $("#page-container").on("scroll", function (e) {
      ...
      const pageIndex = Math.round(
        Math.ceil(initialState.scrollTop / pageHeight)
      ); //
